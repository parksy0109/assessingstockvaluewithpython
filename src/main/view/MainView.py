...
class MainView:
    ...
    def showTkinterTable(self, list, grounName):
        ...
        self.treeview.heading("seven", text="업종 평균 PER 비교 결과", anchor="center")
        #
        ...
        for i in range(len(list)):
            value = [list[i].name, list[i].code, list[i].currentPrice, list[i].priceEarningsRatio,
                     ...
                     list[i].returnOnEquity, list[i].evaluation]
    ...
    def showGraph(self, PyKRXs):
        ...
        for krx in kr_xs:
            ...
            # 그래프 y 축 값
            y1_BPS_list.append(float(krx.BPS))
            y2_PER_list.append(float(krx.PER))
            y3_PBR_list.append(float(krx.PBR))
            y4_EPS_list.append(float(krx.EPS))
            y5_DIV_list.append(float(krx.DIV))
            y6_DPS_list.append(float(krx.DPS))
            y7_CP_list.append(float(krx.currentPrice))

        # plt.plot(x_date_list, y1_BPS_list)  # 첫번째 라인 BPS
        plt.plot(x_date_list, y7_CP_list)  # 두번째 라인 PER
